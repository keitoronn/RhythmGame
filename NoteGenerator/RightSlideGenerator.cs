...
public class RightSlideGenerator : MonoBehaviour// 右スライドノーツを　読み取った楽譜CSVを基に　生成
{
    ...
    void Start()
    {
        timing = TuneInfo.Righttiming;
        place = TuneInfo.Rightplace;
        longnotes = TuneInfo.Rightlongnotes;
        noteend = TuneInfo.Rightnoteend;
    ...
    void Update()
    {
        ...
        if (bluenote < timing.Count)
        {
            if (TuneInfo.ddelta > timing[bluenote] && bluenote < timing.Count)
            {
                ...
                GameObject go = Instantiate(RightSlide) as GameObject;
                int size = 0;
