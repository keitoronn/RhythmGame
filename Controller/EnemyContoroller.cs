...
public class EnemyContoroller : MonoBehaviour//左ノーツの落下、判定、削除の管理
{
    GameObject player;
    GameObject director;
    ...
    void Update()
    {
        ...
        if (Input.GetKey(KeyCode.A) && size == 1)
        {
        ...
        else if (Input.GetKey(KeyCode.A) && size > 1 && flagLong == 0)
        {
            if (flagS == 0 && d <= great && transform.position.x == this.player.transform.position.x -15)
            {
                director.GetComponent<GameDirector>().ScoreUp1();
        ...
        else if (Input.GetKey(KeyCode.A) && flagLong == 1)
        {
            if (dl <= 1 && transform.position.x == this.player.transform.position.x -15)
            {
                director.GetComponent<GameDirector>().ScoreUp1();
                director.GetComponent<GameDirector>().ComboUp();
                director.GetComponent<GameDirector>().MaxShow2();
