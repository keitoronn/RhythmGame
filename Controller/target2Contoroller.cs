...
public class target2Contoroller : MonoBehaviour//右ノーツの落下、判定、削除の管理
{
    GameObject player;
    GameObject director;
    ...
    void Update()
    {
        ...
        //if ((Input.GetKey(KeyCode.Space) || Input.GetMouseButton(1)) && size == 1)
        if ((Input.GetKey(KeyCode.D) || Input.GetMouseButton(1)) && size == 1)
        {
            if (flagS == 0 && d <= great && transform.position.x == this.player.transform.position.x + 15)
            {
            ...
            else if (flagS == 0 && d <= good && transform.position.x == this.player.transform.position.x+15)
            {
        ...
        //else if ((Input.GetKey(KeyCode.Space) || Input.GetMouseButton(1)) && size > 1 && flagLong == 0)
        else if ((Input.GetKey(KeyCode.D) || Input.GetMouseButton(1)) && size > 1 && flagLong == 0)
        {
            if (flagS == 0 && d <= great && transform.position.x == this.player.transform.position.x + 15)
            {
                director.GetComponent<GameDirector>().ScoreUp1();
        ...
        //else if ((Input.GetKey(KeyCode.Space) || Input.GetMouseButton(1)) && flagLong == 1)
        else if ((Input.GetKey(KeyCode.D) || Input.GetMouseButton(1)) && flagLong == 1)
        {
            if (dl <= great && transform.position.x == this.player.transform.position.x + 15)
            {
                director.GetComponent<GameDirector>().ScoreUp1();
                director.GetComponent<GameDirector>().ComboUp();
                director.GetComponent<GameDirector>().MaxShow2();
        ...
        else if (flagpress == 1)
        {
            ...
            else if (dl <= great && transform.position.x == this.player.transform.position.x + 15)
            {
                director.GetComponent<GameDirector>().ScoreUp2();
                director.GetComponent<GameDirector>().ComboUp();
                director.GetComponent<GameDirector>().MaxShow2();
